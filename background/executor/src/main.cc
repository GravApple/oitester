...
/**
 * Get the left part of an equality.
 * `start' specifies the left border we start from.
 */
...
/**
 * Get the right part of an equality.
 */
...
/**
 * Transform a string to a 10-based integer.
 */
...
/**
 * Executor.
 * A utility to safely run and monitor an untrustful program.
 * Usage: executor FILENAME LANGUAGE
 * [ --time_limit=1000 ] [ --memory_limit=256 ] [ --output_limit=10 ]
 * [ --input_file=_.in ] [ --output_file=_.out ]
 * [ --log_file=log ] [ --only_compile?=false ] [ --not_compile?=false ]
 */
int main(int argc, char* argv[]) {
    if (argc < 3) {
        cout <<
            "Usage: executor FILENAME LANGUAGE(cpp, c, pascal)" << endl <<
            "       [ --time_limit=1000 ] [ --memory_limit=256 ] [ --output_limit=10 ]" << endl <<
            "       [ --input_file=_.in ] [ --output_file=_.out ]" << endl <<
            "       [ --log_file=log ]" << endl <<
            "       [ --only_compile?=false ] [ --not_compile?=false ]" << endl;
        return 1;
    }
    ...
    OITesterExecutor::FileLogger logger(args["log_file"]);

    if (args["not_compile?"] == "false") {
        int compile_return_code = OITesterExecutor::compile(
